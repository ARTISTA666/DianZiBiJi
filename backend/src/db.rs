...
const RUST_SCHEMA_VERSION: i32 = 6;
...
pub async fn initialize_database(pool: &PgPool, settings: &Settings) -> Result<(), DatabaseError> {
    // Test suites run dozens of `initialize_database` calls concurrently against
    // one database. The ensure-DDL below takes AccessExclusive locks (ALTER
    // TABLE / CREATE INDEX), which can interleave with other tests' business
    // transactions (audit-log inserts, agent-action inserts, RAG chunk syncs)
    // into a deadlock: PostgreSQL then aborts one participant with 40P01, the
    // losing test panics on unwrap, and a cascade of unrelated setup failures
    // follows. A lock_timeout keeps the ensure transaction a short, retriable
    // critical section instead of queueing behind arbitrary work while holding
    // the advisory init lock; retrying the whole transaction re-runs all the
    // IF NOT EXISTS / IF NOT MATCHES paths, which are idempotent by design.
    for attempt in 0..90u32 {
        let result = initialize_database_once(pool, settings).await;
        match result {
            Ok(()) => return Ok(()),
            Err(DatabaseError::Sql(sqlx::Error::Database(db_error)))
                if db_error.code().is_some_and(|code| {
                    matches!(code.as_ref(), "40P01" | "55P03" | "57014" | "40001")
                }) =>
            {
                let backoff = Duration::from_millis(100 + 40 * (attempt as u64 % 10));
    ...
    Err(DatabaseError::Domain(
        "initialize_database still contended after retries".to_owned(),
    ))
}

async fn initialize_database_once(pool: &PgPool, settings: &Settings) -> Result<(), DatabaseError> {
    // Session-level advisory lock: held across the transaction boundary so the
    // ensure-DDL below never queues for table locks while other initializers
    // already hold the init token (the xact-scoped variant released it only at
    // commit, forcing every waiter into a thundering re-run).
    ...
    sqlx::query("SELECT pg_advisory_lock($1)")
        ...
        .execute(&mut *lock_connection)
        .await?;
    // From here the init token is exclusively ours: no other ensure-DDL can
    // interleave, and AccessExclusive requests now only race short business
    // transactions, which the retry loop below absorbs.
    ...
    .execute(&mut *transaction)
    .await?;
    // Mirrors legacy Alembic migration 0014: formal blind-review batches.
    // The Rust runtime owns this table on fresh databases; the alembic file
    // only mirrors it for the legacy development path.
    sqlx::raw_sql(
        r#"
        CREATE TABLE IF NOT EXISTS public.ai_blind_review_batches (
            id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            project_id integer NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
            experiment_run_id integer NOT NULL REFERENCES public.ai_experiment_runs(id) ON DELETE CASCADE,
            freeze_manifest_sha256 varchar(64) NOT NULL,
            reviewer_a_user_id integer NOT NULL REFERENCES public.users(id),
            reviewer_b_user_id integer NOT NULL REFERENCES public.users(id),
            status varchar(20) NOT NULL DEFAULT 'LOCKED',
            created_by integer NOT NULL REFERENCES public.users(id),
            created_at timestamp with time zone NOT NULL DEFAULT now(),
            unblinded_at timestamp with time zone,
            unblinded_by_user_id integer REFERENCES public.users(id),
            CONSTRAINT uq_ai_blind_review_batches_experiment_run UNIQUE (experiment_run_id),
            CONSTRAINT ck_ai_blind_review_batches_distinct_reviewers
                CHECK (reviewer_a_user_id <> reviewer_b_user_id),
            CONSTRAINT ck_ai_blind_review_batches_status CHECK (status IN ('LOCKED', 'UNBLINDED'))
        );
        CREATE INDEX IF NOT EXISTS ix_ai_blind_review_batches_project
            ON public.ai_blind_review_batches (project_id);
        "#,
    )
    .execute(&mut *transaction)
    .await?;
    ...
    sqlx::raw_sql(
        ...
        -- Mirrors legacy Alembic migration 0016: project alert tables
        -- (创新点四：预警与人工审核闭环——阈值可调的多维指标 + 报警收件箱).
        CREATE TABLE IF NOT EXISTS public.project_alert_thresholds (
            id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            project_id integer NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS uq_project_alert_threshold_metric
            ON public.project_alert_thresholds (project_id, metric);
        ...
        CREATE TABLE IF NOT EXISTS public.project_alerts (
            ...
            created_at timestamp with time zone NOT NULL DEFAULT now(),
            updated_at timestamp with time zone NOT NULL DEFAULT now(),
            CONSTRAINT ck_project_alerts_level CHECK (level IN ('warn', 'critical')),
            CONSTRAINT ck_project_alerts_status
                CHECK (status IN ('open', 'acknowledged', 'resolved'))
        );
        CREATE INDEX IF NOT EXISTS ix_project_alerts_project_status
            ON public.project_alerts (project_id, status);
        CREATE UNIQUE INDEX IF NOT EXISTS uq_project_alerts_open_metric
            ON public.project_alerts (project_id, metric)
            WHERE status = 'open';

        -- Mirrors legacy Alembic migration 0017: agent suggestion feedback
        -- (创新点二：建议采纳留痕——采纳/忽略/过期三态，为行为证据提供直接数据源).
        CREATE TABLE IF NOT EXISTS public.agent_suggestion_feedback (
            ...
            acted_by integer NOT NULL REFERENCES public.users(id),
            acted_at timestamp with time zone NOT NULL DEFAULT now(),
            CONSTRAINT ck_agent_suggestion_feedback_status
                CHECK (status IN ('accepted', 'ignored', 'expired')),
            CONSTRAINT uq_agent_suggestion_feedback_project_key
                UNIQUE (project_id, suggestion_key)
        );
        CREATE INDEX IF NOT EXISTS ix_agent_suggestion_feedback_project
            ON public.agent_suggestion_feedback (project_id);
        "#,
    )
    .execute(&mut *transaction)
    .await?;
