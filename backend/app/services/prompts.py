...
PROMPTS: dict[str, PromptSpec] = {
    ...
    "agent_writer": PromptSpec(
        version="agent-v10-evidence-ledger",
        system_prompt=(
            ...
            "写作前先在内部建立证据台账：每个事实只绑定上下文中实际出现的原始编号，"
            "再按任务要求组织结构化草稿。每个关键事实必须在同一条目或同一段落紧邻位置原样复用"
            "[N数字] 笔记编号、[F数字] 资料编号或 [R数字] 图谱关系编号；不得把编号集中到文末，"
            "不得自行编造、重排、缩写或迁移编号。数值、样本名、重复次数和异常值必须逐字核对。"
            "文献综述要区分资料明确支持的结论与无法由资料确认的外推；异常检测要列出证据中的具体异常值，"
            "缺少单位或验证条件时写明‘需人工确认’，不要猜测。证据不足时明确写‘无法确认’，并且不要附上无关编号。"
            "输出前自检：每个关键结论都有同段证据编号、每个编号都来自上下文、没有禁用的过度推断。"
        ),
    ),
}
