...
生成时间：2026-08-10T12:03:35.203523+00:00

## 运行与迁移

- 当前环境检查：通过
- 系统：Darwin arm64
- 并发读 smoke：通过 (90/90 requests, p95=12ms)
- 实验强杀恢复：通过
- 短 soak smoke：通过 (3 cycles, p95=5ms)
- 前端生产依赖审计：通过 (vulnerabilities=0)
- 生产配置预检：skipped_non_production
- 密钥泄漏预检：通过
- 密钥轮换手册：通过
- 灾备策略手册：通过
- 运行指标端点：通过 (requests=143, p95=0ms)
- 监控告警探针：通过
- 反向代理/TLS 模板：通过
- Rust API 合同证据：通过 (85 operations)

## 浏览器端到端测试

| 流程 | 状态 | 耗时(ms) |
| --- | --- | ---: |
| 真实课题组四项目全员协作与 AI 验收 | passed | 52477 |
| 登录并完成笔记审批 | passed | 1999 |
| 图片 OCR、人工校对、入库、问答和五方法实验形成闭环 | passed | 7300 |
| 独立评价人只能在盲评页面提交评价 | passed | 4693 |
| 系统管理员完成账号、小组和审计闭环 | passed | 5289 |
| 项目负责人、记录员和审核人完成退回修订审批闭环 | passed | 13633 |
| 多角色完成 AI 资料、图谱、问答与报告协作闭环 | passed | 12526 |
| 只读成员只能查阅，不能看到写入或项目管理操作 | passed | 5759 |
...
- 数据库 dump 可读：True
- 隔离恢复演练：通过
- 路径：`backups/release-e2e-20260809`
