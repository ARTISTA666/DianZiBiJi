...
def _generate_query_logs(db: Session, project: Project, admin: User, kg_service: KnowledgeGraphService):
    ...
    for qid, qtype, question in QUESTIONS:
        for mode in ["project_rag", "kg_enhanced_rag"]:
            ...
            # 评价
            score = random.randint(4, 5) if mode == "kg_enhanced_rag" else random.randint(3, 5)
            is_accurate = True if score >= 4 else (random.random() > 0.3)
            is_traceable = True if mode == "kg_enhanced_rag" else (random.random() > 0.2)
